...
class PairTersoff : public Pair {
 ...
 protected:
  ...
  //Edit 1 begin: defining the parameters that need to be accessed in the pair_tersoff file.
  FILE *FfileQ; //Pointer to the file for outputting the modal heat flux values.
  double ***eigen_vector; //eignevector variable
  double *F_n,*Fr_n,*V_n,*Vr_n,*Q_n,Q_n_total;
  // F_n and V_n values are for calculating the modal F values from Eq. X and modal velocity coordinates from Eq. Y. Since these values will be calculated by multiple processors in LAMMPS parallel execution, they will be added up and saved in Fr_n and Vr_n variables using the MPI_reduce command at the end of the time step.
  int Natoms,numconsideredmodes,everythis;
  //Natoms is the total number of atoms in the system
  //numconsideredmodes is the number of modes that are assigned to the computational node that is executing the job now.
  //everythis is for doing the modal anlaysis and recording the modal heat flux values every this time step
  //Edit 1 end
